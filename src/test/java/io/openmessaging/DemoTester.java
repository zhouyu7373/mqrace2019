...
public class DemoTester {
    ...
    public static void main(String args[]) throws Exception {
        ...
        for (int i = 0; i < checkTsNum; i++) {
            checks[i] = new Thread(new ValueChecker(messageStore, maxCheckTime, 30000, msgNum, maxValueCheckSize, valueCheckTimes, valueCheckNum));
        }
        for (int i = 0; i < checkTsNum; i++) {
            checks[i].start();
        }
    ...
    static class MessageChecker implements Runnable {
        ...
        private void checkError(int i,long tMin, long tMax) {
            System.out.println(" message check error" + " tMin " + tMin + " tMax " + tMax);
            System.exit(-1);
        }

        @Override
        public void run() {
            List<Message> msgs1 = messageStore.getMessage(5, 400, 199, 201);
    ...
    static class ValueChecker implements Runnable {
        ...
        @Override
        public void run() {
            long val1= messageStore.getAvgValue(0, 1111111111, 965151, 1017094);
